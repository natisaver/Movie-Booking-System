...
public class ReviewController {
    ...
    public static ArrayList<Review> read() {
        ...
        // If Database Exists
        String line = "";
        ArrayList<Review> movieArrayList = new ArrayList<>();
        try {
            reader.readLine();
            while ((line = reader.readLine()) != null) {
                String[] tokens = line.split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)", -1);
                Review review = new Review(tokens[0].substring(1, tokens[0].length() - 1),
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens[0].substring(1, tokens[0].length() - 1).toLowerCase().equals(title.toLowerCase())) {
                    return new Review(tokens[0].substring(1, tokens[0].length() - 1),
                            tokens[1].substring(1, tokens[1].length() - 1),
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens[0].substring(1, tokens[0].length() - 1).toLowerCase()
                        .equals(review.getTitle().toLowerCase())) {
                    Found = true;
                    writer.close();
                    reader.close();
                    // delete old file
            ...
            if (Found == false) {
                writer.append('"' + review.getMovie() + '"');
                writer.append(",");
                writer.append('"' + review.getDirector() + '"');
                writer.append(",");

                writer.append('"');
                for (int i = 0; i < review.getCast().size(); i++) {
                    writer.append(review.getCast().get(i));
                    if (i == review.getCast().size() - 1)
                        break;
                    writer.append(", ");
                }
                ...
                writer.append(review.getReview().name());
                writer.append(",");
