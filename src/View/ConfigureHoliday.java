      ...
      switch (choice) {
        ...
        case 3:
          ...
          while (!oldDate.matches(dateCheck)
              || !HolidayController.readByDate(LocalDateTime.parse(oldDate + " 00:00", formatter2))) {
            if (oldDate.isBlank()) {
              return this;
        ...
        case 4:
          ...
          if (!byName) {
            System.out.println(ConsoleColours.WHITE_BOLD + "Enter the date of the holiday to be deleted: (yyyy-MM-dd)"
                + ConsoleColours.RESET);
            date = sc.nextLine();
            while (!date.matches(dateCheck)) {
              if (date.isBlank()) {
            ...
            date += " 00:00";
            dateTime = LocalDateTime.parse(date, formatter);
            if (HolidayController.deleteSingleHoliday(dateTime))
              System.out.println(ConsoleColours.GREEN + "Holiday deleted!" + ConsoleColours.RESET);
            else
              System.out.println(ConsoleColours.RED + "Holiday date not found!" + ConsoleColours.RESET);
            System.out.println();
          }

          else {
            System.out.println(
                ConsoleColours.WHITE_BOLD + "Enter the name of the holiday to be deleted:" + ConsoleColours.RESET);
            name = sc.nextLine();
            if (name.isBlank()) {
              return this;
            }
            if (HolidayController.deleteHolidayName(name))
              System.out.println(ConsoleColours.GREEN + "Holiday(s) deleted!" + ConsoleColours.RESET);
            else
              System.out.println(ConsoleColours.RED + "Holiday name not found!" + ConsoleColours.RESET);
            System.out.println();
          }
          break;
